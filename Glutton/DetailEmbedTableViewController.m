...
@property (strong, nonatomic) IBOutlet StyledLabel *zLabel;
@property (weak, nonatomic) IBOutlet UIWebView *webReviews;
...
- (void)viewDidLoad {
    ...
    [self.zLabel setText:@"City, Zip code"];
    ...
    NSString *city = [self.restaurant.location objectForKey:@"city"];
    if(!city){
        city = @"";
    }
    NSString *zip_code = [self.restaurant.location objectForKey:@"postal_code"];
    if(!zip_code){
        zip_code = @"";
    }
    [self.cityLabel setText:[NSString stringWithFormat:@"%@, %@", city, zip_code]];
   ...
   // [self.phoneNumberLabel.titleLabel setFont:[UIFont fontWithName:@"Bariol-Regular" size:15]];
    ...
    [self.webReviews loadRequest:obj]; // Do any additional setup after loading the view, typically from a nib.
