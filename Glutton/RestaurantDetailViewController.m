...
- (void)viewDidLoad {
    ...
    NSArray *address = [self.restaurant.location objectForKey:@"address"];
...
- (IBAction)openMaps:(id)sender {
    ...
    NSString *strAddress;
    if(address && address.count){
       strAddress = [[self.restaurant.location objectForKey:@"address"] objectAtIndex:0];
    }else{
        strAddress = @"";
    }
    UIAlertController *alert = [UIAlertController
                                alertControllerWithTitle:@"Open In Maps?"
                                message:strAddress preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)cmdThumbsUp:(id)sender {
    
    self.restaurant.isSeenDic = @1;
    ...
    int index=0;
    for(int i=0; i<restaruantDict.count; i++)
    {
        dict = [[restaruantDict objectAtIndex:i] mutableCopy];
        NSString *strID = [dict objectForKey:@"id"];
        
        if ([strID isEqualToString:self.restaurant.id] ) {
            [dict setValue:self.restaurant.isSeenDic forKey:@"isseendic"];
            index = i;
            break;
        }
    }
    
    [restaruantDict setObject:dict atIndexedSubscript:index];
...
- (IBAction)cmdThumbsDown:(id)sender {
